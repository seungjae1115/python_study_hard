...
from chaptor09_classes.main import student_info
...
2.소멸자(Destructor)
    인스턴스가 생성할 때 자동으로 생성되는 생성자와 마찬가지로 인스턴스가 소멸될 때 호출되는 메서드 
    이를 소멸자라고 하며, 소멸자를 정의하는 메서드는 __del__()
'''
# class Sample:
#     #생성자 정의
#     def __init__(self):
#         print(f"인스턴스가 생성되었습니다.")   # 생성자가 생성할 때 속성에 값만 대입하는게 아니라 출력문을 삽입할 수도 있음.
#
#     def __del__(self):
...
생성잘ㄹ 이용해서 용량을 가진 usb인스턴스를 만드는 프로그램을 작성하시오 
...
#
# student1.print_profile()
# student1.grade = "말도 안되는 값을 넣더라도 가능"   #직접 입력했기 때문에
# print(student1.get_grade())                    #결과값: 말도 안되는 값을 넣더라도 일단 가능->로 나옴
#
# #이상을 이유로 인스턴스 변수에 값을 대입할 때 제약을 걸기 위해 method를 경유하여 값을 대입하도록
# #권장함.-> setter(call2()유형) / getter(call3()유형)라는 개념 -> 클래스로 돌아가서 해당 메서드 추가하겠음
# student1.set_grade("A+")
# student1.print_profile()
#
# student1.set_grade("안되는거 입력")
# student1.print_profile()

#객체 생성
# student1 = Student("김철수","2025001", "A")
# student2 = Student("이영희", "20250002","B")
# student3 = Student("박민지", "20250003", "C")
#
# #인스턴스 메서드 호출
...
1.Setter / Getter 란?
    ...
    3) 왜 setter getter를 사용하는가?
      (1) 데이터 보호 및 무결성 유지
        :속성 값을 직접 변결하는 경우, 잘못된 값이 입력될 가능성이 높음.
        :Setter를 사용하면 특정 조건을 만족하는 값만 속성에 대입 가능
      (2) 객체의 캠슐화(Encapsulation) 실현
        :객체 내부 데이터를 외부에서 직접 수정하는 것을 방지
        :대신 메서드를 통해서만 접근하도록 제한하여 보안성을 높임
      (3) 추후 유지 보수 및 확정성 용이
        :setter/getter를 사용하면 특정 속성에 대한 로직을 쉽게 변경 가능(if절 바꾸는거 생각하면 됨)
        :예를 들어 특정 속성을 설정할 때 추가적인 검증이 필요하면 Setter에서 처리 가능
2.이제 클래스를 정의할 때 기본적으로 setter getter를 타이핑하면서 배울 예정
      1)Setter: call2()유형 -> 매개변수 o 리턴x 
      2)getter :call3()유형 -> 매개변수 x 리턴o   
...
print(f"나이는 {person.get_age()}인데, 만으로는 {person.get_age()-2}살입니다.")  #이런 이유로 getter 사용
